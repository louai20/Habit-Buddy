...
export function HabitTrackerView({ user, habits, onMarkHabitDone, onUnmarkHabitDone }) {
  ...
  const handleDateSelect = (day) => {
    ...
    if (isCompleted) {
      ...
      // If it's a past date, mark it as uncompleted (orange)
  ...
  const calculateProgress = () => {
    ...
    const totalDays = Math.floor((currentEndDate - startDate) / (1000 * 60 * 60 * 24)) + 1;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.habitInfo}>
        ...
        <Text style={styles.habitFrequency}>Frequency: {habitFromRoute.frequency}</Text>
        ...
        <Text style={styles.habitProgress}>
          Completed: {Object.keys(markedDates).filter(
            date => markedDates[date].selectedColor === '#059669'
          ).length} times
        </Text>
      </View>
